...
def main():
    ...
    # --------- 7.2 先加载并统计数据 ---------
    print("┌──────────────────────────────────────────────────┐")
    train_samples, train_total, train_counts = load_and_summarize_cnews(
        TRAIN_FILE, label2id, clean_text, skip_invalid=True, min_chinese_ratio=0.3
    )
    print(f"Train 总样本数: {train_total}")
    ...
    # --------- 7.3 构建 Dataset 与 DataLoader ---------
    tokenizer = BertTokenizer.from_pretrained(BERT_PATH)

    print("正在构建 train 数据集...\n")
    train_dataset = CNewsDataset(train_samples, tokenizer, MAX_SEQ_LEN, num_classes=NUM_CLASSES)
    print("正在构建 val 数据集...\n")
